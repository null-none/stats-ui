(function (global) {
  /**
   * Scatter.render(svgSelector, data, options)
   *
   * data – [{ x, y, label?, color?, r?, group? }]
   *
   * options – {
   *   width, height, margin,
   *   xLabel, yLabel,
   ...
   *   refLines,            // [{ axis:"x"|"y", value, label }] — reference lines
   * }
   */
  ...
  function render(svgSelector, data, options = {}) {
    ...
    (options.refLines || []).forEach((rl) => {
      if (rl.axis === "x") {
        g.append("line")
          .attr("x1", x(rl.value)).attr("y1", 0)
          .attr("x2", x(rl.value)).attr("y2", H)
          .attr("class", "stats-scatter-refline");
        if (rl.label)
          g.append("text").attr("x", x(rl.value) + 4).attr("y", 10)
            .attr("class", "stats-scatter-reflabel").text(rl.label);
      } else {
        g.append("line")
          .attr("x1", 0).attr("y1", y(rl.value))
          .attr("x2", W).attr("y2", y(rl.value))
          .attr("class", "stats-scatter-refline");
        if (rl.label)
          g.append("text").attr("x", 4).attr("y", y(rl.value) - 4)
            .attr("class", "stats-scatter-reflabel").text(rl.label);
      }
    });
    ...
    if (options.xLabel) {
      g.append("text")
        ...
        .attr("class", "stats-scatter-axislabel")
        .text(options.xLabel);
    }
    if (options.yLabel) {
      g.append("text")
    ...
    if (options.showLabels !== false) {
      g.selectAll(".stats-scatter-label")
        .data(data.filter((d) => d.label))
        .enter()
        .append("text")
        .attr("x", (d) => x(d.x) + (d.r || 6) + 4)
        .attr("y", (d) => y(d.y) + 4)
        .attr("class", "stats-scatter-label")
        .text((d) => d.label);
    }
  }

  global.Scatter = { render };
})(window);
