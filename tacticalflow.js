(function(global) {
  function renderTacticalFlow(svgSelector, data, options = {}) {
    ...
    const margin = options.margin || { top: 40, right: 20, bottom: 30, left: 50 };
    ...
    legend.append("rect").attr("x", -30).attr("y", -10).attr("width", 120).attr("height", 25).attr("fill", "white").attr("opacity", 0.7);

    legend.append("circle").attr("cx", 0).attr("cy", -2).attr("r", 5).attr("fill", options.attackColor || "black");
    legend.append("text").attr("x", 10).attr("y", 4).text(options.attackLabel || "Attack");

    legend.append("circle").attr("cx", 80).attr("cy", -2).attr("r", 5).attr("fill", options.defenseColor || "red");
    legend.append("text").attr("x", 90).attr("y", 4).text(options.defenseLabel || "Defense");
  }
