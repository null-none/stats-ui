...
function render(svgSelector, data, options = {}) {
  ...
  g.append('text').attr('x', x(45) + 4).attr('y', 12)
    .attr('class', 'stats-xgchart-halftime-label').text('HT');
  ...
  const lineGen = d3.line()
    .x((d) => x(d.minute)).y((d) => y(d.cumXg)).curve(d3.curveStepAfter);
  ...
  [
    ...
    g.selectAll(null).data(pts.filter((d) => d.shot)).enter().append('circle')
      .attr('cx', (d) => x(d.minute)).attr('cy', (d) => y(d.cumXg))
      .attr('r', 5).attr('fill', color).attr('class', 'stats-xgchart-dot');

    const last = pts[pts.length - 1];
    g.append('text').attr('x', x(last.minute) + 4).attr('y', y(last.cumXg) + 4)
      .attr('class', 'stats-xgchart-endlabel').attr('fill', color)
  ...
  [{ label: homeL, color: homeC }, { label: awayL, color: awayC }].forEach(({ label, color }, i) => {
    lg.append('line').attr('x1', i * 120).attr('y1', 0).attr('x2', i * 120 + 16).attr('y2', 0)
